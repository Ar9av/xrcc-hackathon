...
function Scene({ isPaletteOpen, onTogglePalette }: SceneProps) {
  ...
  const preloadedScenes = useMemo(() => {
    ...
    // Measure bounding boxes to help with scale debugging
    const measureModel = (scene: THREE.Object3D, name: string) => {
    ...
    measureModel(bedGltf.scene, 'Bed')
    measureModel(sofaGltf.scene, 'Sofa')
    measureModel(tableGltf.scene, 'Table')
